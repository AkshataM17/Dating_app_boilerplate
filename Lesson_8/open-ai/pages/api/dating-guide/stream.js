...
export default authenticateSSE(async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      ...
      res.write(
        `data: ${JSON.stringify({ error: "Error generating advice" })}\n\n`
      );
      res.end();
    }
  } else {
